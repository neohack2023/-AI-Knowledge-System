...
| `bootstrap-devos` | Knowledge Steward | inspect a target repository and compile the smallest non-destructive repository-native DevOS scaffold when no equivalent operating layer exists |
...
- `bootstrap-devos` must reuse an existing DevOS/equivalent repo OS and must never install a second authority-shaped tree merely for naming consistency.
- `bootstrap-devos` may install a bounded research-preflight policy, but research remains authority-neutral and candidate-only until evaluated and governed.
...
The lifecycle procedures, adaptive DevOS bootstrap, and governance synchronization meet that bar because they have stable inputs, outputs, evidence rules, and stop conditions.
...
7. for `bootstrap-devos`, preserve plan-first/non-destructive behavior and do not force mature-reference machinery onto smaller targets;
8. for `sync-governance`, keep receipt/freshness validation deterministic and never let external-fetch success alone renew the lock.
